...
export class Quest extends Component {
  ...
  deleteChallenge = id => {
    ...
    quest.challenges.splice(
      quest.challenges.findIndex(chal => chal.id === id),
      1
    );
    this.updateQuest(quest);
  };
  ...
  updateChallenge = challenge => {
    const quest = { ...this.props.data };
    const chalIndex = quest.challenges.findIndex(
      chal => chal.id === challenge.id
    );
  ...
  render() {
    ...
    const completed =
      this.state.showCompleted && completedChallenges.length ? (
        <>
